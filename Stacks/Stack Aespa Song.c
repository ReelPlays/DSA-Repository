...
int main(){
    ...
    printf("Peek (Top Song): %s (%d)", topSong.songTitle, topSong.releaseYear);
...
Album initializeAlbum(Album *ae){
	ae->top = NULL;
}
...
void populateArtist(Album *ae, Artist aes){
	ae->artistInfo = aes;
}
...
void pushSong(Album *ae, Song newSong){
	SongNode* temp = malloc(sizeof(struct SongNode));
    if(temp != NULL){
